...
class FacebookConfig:
    ...
    def __init__(self, config):
        ...
        try:
            ...
            logger.error("##Error reading facebook db config file: %s" %str(e))
...
class SpoolSendFacebook(spooler.Spooler):
    ...
    def __init__(self):
        ...
        logger.info("##Configuration loaded: %s" %self.facebook_config.hour_limit)
        try:
            self.facebook_accounts = utilities.read_config_file(self.facebook_config.accounts_file)
        except Exception as e:
    ...
    def spool(self, addresses, content):
        logger.info("##Start spool() for SpoolSendFacebook with: %s" %(addresses))
        ...
        logger.debug("Event info to facebook post: %s" %(bulletin_object.plain))
        ...
        ##The filter is in charge of create a JPG image in case eqevents is not ready.

        event_image_path = "%s/%s/%s-map.png" %(self.facebook_config.eqevent_path,event_id,event_id)

        if not os.path.isfile(event_image_path):
             event_image_path = "%s/%s/%s-map.jpg" %(self.facebook_config.eqevent_path,event_id,event_id)


        event_dict = {'text':'%s' %bulletin_object.plain, 'image_path':event_image_path}
        ...
        if not self.check_antiquity(event_datetime):
            logger.info("event too old. Limit is %s hours" %self.facebook_config.hour_limit)
            return True

        for address in addresses:
            """Check against the DB if the event has been published already"""        
            select = "*"
            where = "event_id='%s' AND status='%s' AND gds_target='%s'" % (event_id, event_status, address[1])
            ...
            try:
                ...
                if facebook_id == False:
                    ...
                    event_row = {'event_id':event_id, 'facebook_id': facebook_id['post_id'] , 'status': event_status, 'gds_target': address[1]}
    ...
    def post_event(self,facebook_api,event_dict):
        ...
        try:
            post_id = facebook_api.put_photo(image=open(event_dict['image_path'],'rb'), message=event_dict['text'])
            return post_id
        except Exception as e:
    ...
    def check_antiquity(self, limit_date_time):
        ...
        """
        date_check = datetime.now() - timedelta(hours=self.facebook_config.hour_limit)
