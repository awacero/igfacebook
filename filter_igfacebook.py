...
class FacebookFilter(Filter):
    ...
    def parseEventParameters(self, event_parameter):
        ...
        if origin:
            ...
            event["time_local"] = self.get_local_datetime(
            ...
            try:
                ...
                pass
            try:
                event["status"] = "%s" % seiscomp3.DataModel.\
                                  EEvaluationModeNames.name(
                                  event_parameter.origin(0).evaluationMode())
            except:
            ...
            event["nearest_city"] = utilities.get_closest_city(
                                    origin.latitude().value(),
                                    origin.longitude().value())
            event["survey_url"] = str(utilities.get_survey_url(
                                  self.get_local_datetime(event['time']),
                                  event['id']))
            event["event_country"] = utilities.get_message_by_country(
                                     origin.latitude().value(),
                                     origin.longitude().value())
    ...
    def get_local_datetime(self, datetime_utc_str):
        ...
        datetime_EC = datetime_UTC.replace(
                      tzinfo=pytz.utc).astimezone(local_zone)
        return datetime_EC
